...
def extrair_todos_dados(texto, filename, pdf_path, temp_dir):
    ...
    dados = {
        ...
        '_Autuações_Count': 0  # Adicionado campo para contagem de autuações
    }
...
def gerar_relatorio_completo(df):
    ...
    for _, row in df_validos.iterrows():
        ...
        # Autuações - CORREÇÃO: Verifica se o valor existe e é válido antes de converter para int
        autuacoes_count = 0
        if '_Autuações_Count' in row and pd.notna(row['_Autuações_Count']) and str(row['_Autuações_Count']).strip() != '':
            ...
            # Fallback para a verificação binária original
